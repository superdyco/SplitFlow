 ...
 * 沒有 uid、沒有誰付的、沒有怎麼分的。
 *
 * 名稱本來刻意不放（怕「阿明的點心」這種只有自己人看得懂、又帶到人的字串
 * 跟著連結傳出去），改放地點。但地點整區列在「去過的地方」了，時間軸再列
 * 一次等於同一份資訊講兩遍，那一區就只剩時間跟金額能看。名稱才是「這天怎麼
 * 過的」真正的答案。
 *
 * 代價要講清楚：**支出名稱會出現在公開連結上**。名稱是使用者自己打的，
 * 產生報告的人要知道這件事 —— 報告頁的產生流程上有提示。
 *
...
export interface ReportEntry {
  ...
  name: string;
  /**
   * 地點名稱。**只有這個改動之前產生的報告有這個欄位**，新報告不再寫入 ——
   * 地點已經整區在「去過的地方」了。留著型別是因為舊報告要照樣打得開。
   */
  place?: string | null;
  /** 主要幣別最小單位整數。 */
  amount: number;
