...
*/
/*
  儲值（purchaseCredits、appStoreNotifications、onPlayNotification）在
  `purchase/functions.ts`，**刻意還沒匯出，連 import 都沒有**。

  它們用到 APPSTORE_* 四個 secrets，而 defineSecret 在模組一載入就會登記 ——
  只要這個檔案 import 了它，部署任何一支函式都會先去查 Secret Manager，而這個
  專案還沒啟用它，結果是**全部函式都部署不上去**。

  辦好 Apple Developer Program 與 Play Console、啟用 Secret Manager、設好四個
  secrets 之後（見 docs/superpowers/plans/2026-09-13-ai-credit-purchase.md 的 Task 11），
  把下面這行的註解拿掉再部署：

  export { purchaseCredits, appStoreNotifications, onPlayNotification } from "./purchase/functions.js";
*/
export {
  adminOverview,
  adminUsers,
