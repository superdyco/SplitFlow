...
3. 排程與每日彙總，加上索引（等它建完，大集合會跑一陣子）
4. 其餘 callable 與頁面：使用者、任務、公開報告、系統健康、稽核日誌
...
`adminLogs` 的寫入路徑已經在 `requireAdmin` 裡（擋下來的存取會記一筆
`denied.access`），但**檢視類的日誌還沒有東西可寫** —— 那要等
`adminUser` / `adminTask` 那幾支。
