...
> **實作後補充（2026-08-04）**：這個決定必須在畫面上明講。第一版做完之後
> 使用者以為「選了照片就上傳了」，因為選完就出現縮圖，看起來跟已經存好的
> 完全一樣。延後上傳是對的，但沉默的延後是在騙人 —— 沒儲存就離開頁面，
> 照片會無聲消失。所以 `unsaved` 是獨立狀態，縮圖上掛「未儲存」標籤，
> 下面直接寫出要按哪一顆按鈕。
...
縮圖右上角掛標籤：未儲存是主色「未儲存」，待上傳是灰色「待上傳」，失敗是紅色
「上傳失敗」（旁邊有可點的重試，重設 `attempts` 再 flush 一次）。已上傳不掛標籤 ——
沒有消息就是好消息。

**這五個狀態每一個都要真的算得出來。** 第一版把 `failed` 寫進型別卻沒有任何分支
會回傳它，結果是：上傳失敗五次之後停止自動重試，畫面卻還顯示「待上傳」，而重試
按鈕只在 `failed` 時渲染 —— 使用者被永久卡住且無法自救。狀態機的每個值都要有
對應的計算路徑，否則型別只是裝飾。

同理，`useReceipt` 的 `error` 必須真的傳進元件並顯示。第一版它存在但沒有任何地方
渲染，連壓縮失敗都是靜默的，違反下面「邊界情況」自己訂的規則。
