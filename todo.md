...
`/tasks/:taskId/expenses/:expenseId`（`ExpenseDetailPage`）。

原本的問題：編輯頁只讓「自己建的、自己先付的、或管理員」進得去，其他人連看都
看不到。列表標了「📎 有收據」，點下去卻沒有東西 —— 而收據的用途就是對帳，
看不到照片等於這個功能對半數的人不存在。

- `ExpenseRow` 現在每個人都點得進去，差別只在點到哪：管得動的進編輯頁，
  其他人進這一頁。順便把那個元件裡一模一樣的兩份模板合成一份。
- 路由的 `/edit` 要排在 `:expenseId` 前面，不然 "edit" 會被當成 id 吃掉。
- 這一頁不寫入，所以不判斷 `taskIsActive`：封存的任務照樣讀得到。
- 收據載入失敗不擋整頁，其他欄位照樣有對帳價值。待上傳的收據（`path` 還是
  null）會明說「還在拍攝者手機裡」，而不是留一個永遠轉圈的區塊。
- 備註在列表被截成一行，這一頁是唯一看得到全文的地方，所以不截斷。
- 分攤依金額由大到小排，對帳時想找的通常是「誰分最多」。

`ReceiptField` 那個留了很久的 `canManage` prop 最後沒用上 —— 這一頁純唯讀，
直接顯示圖片就好，不需要一個「不能操作的操作元件」。

## 已完成：App 的支出詳情頁與權限分流

把上一節那一頁搬到 Flutter，順便修掉兩個查出來的問題。

**原本比網頁版更糟。** 網頁版是「沒權限就進不去編輯頁」，App 那邊是
`onTap` 完全沒判斷權限（只看封存），任何人都能打開表單、改欄位、按儲存，
然後被 Firestore rules 擋下來。是**填完才發現不行**。

- `canManageExpense()` 進 `domain/expense_actions.dart`，照
  `payment_actions.dart` 的模式：權限是**規則**不是畫面，rules 擋的是同一組
  條件，抽成純函式才能用測試釘住。列表與表單頁共用它。
- 列表依它分流：管得動的進編輯頁，其他人進詳情頁。因為封存已經包在函式裡，
  封存的任務自然變成所有人都進詳情頁 —— 之前是整列點不動，而封存的旅程
  正是最需要翻帳的時候。
- 表單頁自己也擋一次（`_Blocked`）。列表分流之後這一段看似多餘，但少了它，
  以後任何一個新入口導進來就又會出現「填完才失敗」。
- 詳情頁**不依 id 重讀，直接收列表已經有的那個 `Expense`**。網頁版要重讀是
  因為它的入口是一條 URL，重新整理之後手上什麼都沒有；App 的導航本來就帶著
  物件走，重讀只是多一趟網路、多一個載入狀態，離線時還會壞。
- `_RemoteReceipt` 從 `receipt_field.dart` 提成公開的 `RemoteReceipt`。
  詳情頁純唯讀，不需要整個 `ReceiptField`。

**過程中查出 `Expense` 模型少了兩個欄位**，兩個都是這次要用的：

- `createdBy` —— 沒有它就判斷不了「這筆是不是我記的」。
- `note` —— **這個是資料流失**。表單存檔時一律寫入 `note` 欄位
  （`'note': _note.text.trim()`），但模型讀不回來、編輯時也就填不進去，
  所以**在 App 裡編輯一筆有備註的支出，按下儲存就把備註洗成空字串**。
  補上欄位、mapper 讀回來、`initState` 回填，三件一起才算修好。
  `mappers_test.dart` 加了一條專門釘這件事。

順便補上「再記一筆」：`repeatFieldsOf()` 早就寫好也有測試，但 App 裡
沒有任何 UI 用到它。Flutter 沒有網頁版 stretched link 那個麻煩，巢狀按鈕
本來就會吃掉點擊，不用擋冒泡。`ExpenseFormPage` 多一個 `repeat` 參數
（網頁版是 `?from=` 再讀一次來源，App 直接把物件帶過去）。
**自訂分攤的金額刻意不帶** —— 那些數字是跟著來源那筆的總額算出來的，
而新的一筆總額還沒填，預填進去只會讓驗證對著使用者沒打過的數字喊對不上。

## 已完成：App 的支出地圖與列表標記

補上手機版跟網頁版對照後缺的兩件小事，都在支出頁籤。

- **清單／地圖切換**。網頁版早就有，App 這邊的地圖只用在單一地點的預覽。
  一趟旅程的支出標在同一張圖上，多個標記時自動框住全部。
  `expenseMarkers()` 與 `markerBounds()` 進 `domain/expense_markers.dart` ——
  「哪些支出畫得上」跟「相機要框多大」是規則不是畫面，放 domain 才測得到
  （widget 那層要跑得動原生地圖 view 才驗得了）。
- 切換鍵**只在畫得出東西時才出現**：沒設 `MAPS_API_KEY`、或一筆有座標的
  支出都沒有，就不給按 —— 按下去只會看到空白。標記數也印在按鈕上，
  因為它跟支出筆數不一樣，不講的話使用者會以為地圖漏了幾筆。
- `PlaceMap` 改成收一組標記，單點那兩個呼叫端走 `PlaceMap.single`。
  多標記時在 `onMapCreated` 之後**等 300ms 才框範圍** —— 地圖剛建好時原生
  那邊還沒量到大小，直接動相機會丟 `Map size should not be null`。
  框不到就停在初始位置，那不影響任何資料，所以失敗吞掉。
- 列表那一行補上 `📍 地點` 與 `📝 備註`（各截一行）。理由跟網頁版當初一樣：
  要點進去才看得到的話，對帳時每一筆都得點一次，那兩個欄位等於白填。

## 已完成：App 的公開旅費報告、探索與收藏

網頁版有、手機版整組沒有的最大一塊。`task_page.dart` 原本寫著
「報告與分享那一整區**不搬**（見 README 的範圍決定）」，但 README 裡
從來沒有那段決定 —— 那行註解指向一份不存在的文件。現在補齊了。

- **Firestore rules、indexes 與 storage.rules 一個字都沒動。** 報告文件、
  收藏、collection group 索引、公開的 map.png 全都是網頁版當初就建好的，
  App 讀寫的是同一批文件。這也是為什麼 mapper 的欄位清單有測試釘住 ——
  兩邊寫出來的形狀必須一樣。
- 算數字的四支（`tripSummary` / `placeTotals` / `reportTimeline` /
  `visiblePlaces`）照網頁版搬進 domain，缺匯率的支出四邊一致排除。
  網頁版的測試案例一條一條跟著搬（`dart test` 多了 60 幾條）。
- **`reportTimeline` 的排序多了一道 tie-break**：網頁版靠
  `Array.prototype.sort` 的穩定性讓「同一天都沒記時間的照記帳先後排」成立，
  **Dart 的 `List.sort` 不保證穩定**，所以這裡自己帶了加入順序當最後一個
  比較欄位。直接照抄的話那個順序會隨資料筆數飄。
- 地圖一樣是產生當下拍的一張靜態 PNG（Maps Static API），公開頁不載 SDK、
  不帶金鑰。金鑰跟 `PlaceMap` 共用同一個 `--dart-define=MAPS_API_KEY`；
  沒帶就沒有地圖，報告其他內容照常產生。
- **地圖失敗一律不擋報告**，但要講得出原因：沒金鑰、地點沒座標、API 沒開通、
  圖太大、上傳失敗各有各的說法，跟報告本身的錯誤分開顯示 —— 混在一起會讓人
  以為整份報告失敗了。
- 產生／撤銷只有 owner 做得到（`canShareReport`，rules 同一條）。
  **刻意不看封存狀態**：報告本來就是旅程結束後才做的。
- 「連結有效」與「列進探索」是兩個開關：前者是「拿到連結的人看不看得到」，
  後者是「陌生人找不找得到」。關連結會一併取消公開，而連結關著時
  「列進探索」整顆停用 —— 列出去只會是一張點進去讀不到的卡片。
- 分享區在連結關著時不顯示分享／複製／開啟。網頁版當初踩過這個坑：
  owner 是成員，所以他自己讀得到已關閉的報告，給了按鈕他會以為連結還通著。
- 探索與收藏放在「我的分帳」的標題列，不混進任務列表 —— 那是別人的旅程。
- **報告連結（`/r/:taskId/:reportId`）也吃 App Link 了**，跟邀請連結同一個
  intent-filter 多一條 pathPrefix。裝了 App 的人點到轉傳來的報告，直接在
  App 裡打開；沒裝的照樣走網頁版。報告頁不需要登入，跟網頁版一致。
- 收藏存的是**收藏當下的快照**（`favoriteToMap` 只有 9 個欄位，有測試釘住
  清單）。報告以後多了什麼都不該自動跟著跑進使用者的收藏裡。

網頁版報告頁那顆返回鍵在 App 上不用做 —— 那是為了「裝成 App 之後沒有上一頁」
才補的，原生版本來就有系統返回。「產生於 X」照樣印：報告是快照，
讀的人要看得出這份是什麼時候的。

## 已完成：App 的診斷資訊與錯誤記錄

網頁版做這個的理由寫著「手機上打不開 console」，結果只有網頁版有。
原生版更嚴重：連版本號都沒有，而 App 不像網頁能靠重整拿到新版。

- `debug_log.dart` 與 `diagnostics.dart` 照網頁版搬，最多 50 筆、**只在記憶體
  裡**、連續重複的併成一筆記次數（重試迴圈會在幾秒內灌滿清單，把最舊、
  也最接近起因的那幾筆擠掉）。網頁版的測試案例跟著搬。
- `describeError` 在 Dart 這邊多擋一件事：預設 `toString()` 會印出
  `Instance of 'Foo'`，那是 Dart 版的 `[object Object]` —— 至少要留下型別名稱。
- **`main()` 掛上兩個全域入口**：`FlutterError.onError`（框架內）與
  `platformDispatcher.onError`（框架外的 Future 與平台通道）。兩個都只記錄、
  不彈 UI —— 這裡抓到的照定義就是沒人接住的錯誤，對使用者說不出有用的話。
  覆寫前先接住 `FlutterError.presentError` 再自己呼叫，不然 console 輸出
  會被一起蓋掉。
- **欄位跟網頁版不一樣，因為故障不一樣**：沒有「待上傳收據」（原生版刻意
  沒有離線佇列，沒那個狀態就沒那條線索）、沒有「啟動方式」（只有一種），
  多了推播 token —— 那是原生獨有的失敗，而且從畫面上完全看不出來。
  「拿不到 token」與「問不到（沒有 Google Play 服務）」分成兩句話。
- 順手把三個原本靜默的失敗接上：註冊推播 token、刪 Storage 上的收據、
  讀報告。前兩個本來就是刻意吞掉的（不該擋住使用者），但吞掉之後
  「都收不到通知」「孤兒檔案愈積愈多」就永遠查不出來。
- **只講金鑰有沒有設定，絕不印出金鑰本身。**
- 版本戳走 `--dart-define=APP_VERSION`，CI 的 iOS build 帶
  `短 commit + 日期`。沒帶的話畫面上寫「未帶版本號（開發建置）」，
  不會假裝自己知道。

## 已完成：App 的支出名稱語音輸入

網頁版 `useDictation` 包的是 Web Speech API，原生這邊是 `speech_to_text`
（Android 的 `SpeechRecognizer`／iOS 的 `SFSpeechRecognizer`）。

- 收尾規則與錯誤翻譯抽進 `domain/dictation.dart` 才測得動：**只削句尾的標點**
  （中文辨識很愛補一個句號），中間的留著 —— 那是使用者真的講出來的。
- **講出來的內容直接取代欄位**，不是接在後面。「晚」加上聽到的「晚餐」會變成
  「晚晚餐」，比重打一次還煩。理由跟網頁版當初一樣。
- 錯誤碼分成「要講」與「不要講」兩類：講完停頓造成的 `error_speech_timeout`
  與連按兩下的 `error_client` **不跳紅字** —— 那不是故障。
  權限、沒有中文語音資料這種則要說得出下一步怎麼做。
- **初始化延到第一次按下麥克風**：多數人記一筆帳根本不會用到語音，
  沒必要每次開表單都去碰原生的辨識服務。
- 服務不可用時**一定要講話**。按了什麼都沒發生是最糟的失敗方式，
  使用者會一直按然後以為整個 App 壞了。
- 麥克風權限不主動要，由系統在按下的當下跳 —— 那時使用者才知道自己在授權什麼，
  比開 App 就問成功率高得多。同樣的理由寫在推播那邊。
- Android 11 起要在 `<queries>` 宣告 `android.speech.RecognitionService`，
  少了它會回報「這台裝置沒有辨識服務」，而實際上是有的。
...
- **iOS standalone 的 sticky 位置會過期**：加到主畫面後開支出表單，選完照片、
  按完 AI，送出列會停在版面變高之前的位置（浮在畫面中間、蓋住備註），捲一下
  才回到底部。修法是 `src/utils/stickyNudge.ts` —— 在那幾個時機捲 1px 再捲回來，
  逼瀏覽器重算。之後如果表單再加會改變高度的東西，記得也呼叫它。

## 已完成：成員可以自己退出任務

一般成員與管理員可以自己退出任務，成員列上顯示「（已退出）」。

- **走 `leaveTask` callable，規則一個字都沒動。** 退出要把自己從 `task.memberIds`
  拿掉，而那個陣列同時是權限清單 —— 規則裡曾經有一條讓成員自己改它的路
  （`updatesSelfMembershipOnly`），那是個洞，早就刪掉了。判斷在 `functions/src/leave.ts`。
- **擁有者不能退出**：退了就沒有人管得了那個任務。他只能封存或刪除整個任務
  （目前沒有手動轉移擁有者的介面）。
- **帳目全部留著**：支出、分攤、付款都不動，他付過的錢與別人欠他的錢照樣算得出來。
- 成員文件記 `left: true`，跟被管理員移除（只有 `active: false`）分開 ——
  對留下來的人是兩件事。「已刪除」壓過「已退出」，「已退出」壓過「已離開」。
- 退出時角色一起降回成員，所以管理員不必先降級。
- 封存的任務不能退出（封存後成員不該變動），已經退出過再按一次算成功。
