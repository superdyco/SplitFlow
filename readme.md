...
量測的結論：進「我的任務」會週期性地卡 **30 秒**，而且卡的時候一定是 30 秒
（29.6 / 30.08 / 30.15 / 30.17，四筆聚在 0.5 秒內），健康時同一個查詢是 49～133ms。
那是逾時的形狀，不是慢的形狀。

**什麼情況會讓連線死掉，目前還不知道。** 已知排除兩個：

- 不是離線快取 —— 換成 `memoryLocalCache` 部署實測過，照樣卡。
- **不是從背景回來** —— 這是原本的假設，量了之後推翻：卡住的樣本 `hiddenMs` 是 0
  （整段都在前景），而背景 52 秒回來的樣本只花 72ms。目前只知道它成群出現。

已知的機制是這一段：`getDocs` 內部是「開一個暫時的監聽器等伺服器同步」，它帶著
`waitForSyncWhenOnline: true`，只要 SDK 認為自己在線上就壓著本機快取不發：
...
**實測驗證過**：補救觸發的兩筆，query 從 29,649～30,171ms 變成 2,508 / 2,509ms，
而且那兩筆是三批資料以來 `fromCache` 第一次出現 `true` —— 快取一直都在，
只是被摀著嘴，狀態一翻成 Offline 就吐出來了。整條推論鏈到此閉合。

門檻一開始設 2.5 秒，驗證過之後調到 1.5 秒：21 筆健康樣本最慢 133ms（含 chunk
下載的冷啟動），一次誤觸發都沒有，而失敗樣本全部在 30 秒那一群，中間完全沒有
東西。**但這是在家裡的網路上量的** —— 出國漫遊時一趟合法查詢可能超過 1.5 秒，
那時會誤觸發（後果是顯示快取 + 重建連線，不算災難但有成本）。等有國外樣本再調。

補救是全域的（整個 app 的 Firestore 一起切離線再接回來），所以只在真的卡住時才跑，
而且同一時間只跑一輪。驗收看報告的「卡住補救 × query」與觸發率。
